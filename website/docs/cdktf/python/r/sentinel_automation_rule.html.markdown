...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        azurerm_sentinel_automation_rule_example =
        azurerm.sentinel_automation_rule.SentinelAutomationRule(self, "example_4",
            action_incident=[SentinelAutomationRuleActionIncident(
                order=1,
                status="Active"
            )
            ],
        ...
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        azurerm_sentinel_automation_rule_example.override_logical_id("example")
```
