...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        azurerm_synapse_integration_runtime_self_hosted_example =
        azurerm.synapse_integration_runtime_self_hosted.SynapseIntegrationRuntimeSelfHosted(self, "example_6",
            name="example",
            synapse_workspace_id=cdktf.Token.as_string(azurerm_synapse_workspace_example.id)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        azurerm_synapse_integration_runtime_self_hosted_example.override_logical_id("example")
```
