...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        azurerm_kubernetes_cluster_extension_example =
        azurerm.kubernetes_cluster_extension.KubernetesClusterExtension(self, "example_2",
            cluster_id=cdktf.Token.as_string(azurerm_kubernetes_cluster_example.id),
            extension_type="microsoft.flux",
            name="example-ext"
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        azurerm_kubernetes_cluster_extension_example.override_logical_id("example")
```
