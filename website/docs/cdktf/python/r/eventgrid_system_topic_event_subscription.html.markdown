...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        azurerm.eventgrid_system_topic.EventgridSystemTopic(self, "example_3",
            location="Global",
            name="example-system-topic",
            resource_group_name=cdktf.Token.as_string(azurerm_resource_group_example.name),
            source_arm_resource_id=cdktf.Token.as_string(azurerm_resource_group_example.id),
            topic_type="Microsoft.Resources.ResourceGroups"
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        azurerm_eventgrid_system_topic_example.override_logical_id("example")
        azurerm_eventgrid_system_topic_event_subscription_example =
        azurerm.eventgrid_system_topic_event_subscription.EventgridSystemTopicEventSubscription(self, "example_4",
            name="example-event-subscription",
            resource_group_name=cdktf.Token.as_string(azurerm_resource_group_example.name),
            storage_queue_endpoint=EventgridSystemTopicEventSubscriptionStorageQueueEndpoint(
                queue_name=cdktf.Token.as_string(azurerm_storage_queue_example.name),
                storage_account_id=cdktf.Token.as_string(azurerm_storage_account_example.id)
            ),
            system_topic=cdktf.Token.as_string(azurerm_eventgrid_system_topic_example.name)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        azurerm_eventgrid_system_topic_event_subscription_example.override_logical_id("example")
```
