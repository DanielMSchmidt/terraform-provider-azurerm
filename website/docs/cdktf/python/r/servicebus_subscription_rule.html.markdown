...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        azurerm_servicebus_subscription_rule_example =
        azurerm.servicebus_subscription_rule.ServicebusSubscriptionRule(self, "example_4",
            filter_type="SqlFilter",
            name="tfex_servicebus_rule",
            sql_filter="colour = 'red'",
            subscription_id=cdktf.Token.as_string(azurerm_servicebus_subscription_example.id)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        azurerm_servicebus_subscription_rule_example.override_logical_id("example")
```
