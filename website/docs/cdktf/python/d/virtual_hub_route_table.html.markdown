...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_virtual_hub_route_table_example =
        azurerm.data_azurerm_virtual_hub_route_table.DataAzurermVirtualHubRouteTable(self, "example",
            name="example-hub-route-table",
            resource_group_name="example-resources",
            virtual_hub_name="example-hub-name"
        )
        cdktf.TerraformOutput(self, "virtual_hub_route_table_id",
            value=data_azurerm_virtual_hub_route_table_example.id
        )
```
