...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_postgresql_flexible_server_example =
        azurerm.data_azurerm_postgresql_flexible_server.DataAzurermPostgresqlFlexibleServer(self, "example",
            name="existing-postgresql-fs",
            resource_group_name="existing-postgresql-resgroup"
        )
        cdktf.TerraformOutput(self, "id",
            value=data_azurerm_postgresql_flexible_server_example.id
        )
```
