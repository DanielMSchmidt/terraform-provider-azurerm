...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_postgresql_server_example =
        azurerm.data_azurerm_postgresql_server.DataAzurermPostgresqlServer(self, "example",
            name="postgresql-server-1",
            resource_group_name="api-rg-pro"
        )
        cdktf.TerraformOutput(self, "postgresql_server_id",
            value=data_azurerm_postgresql_server_example.id
        )
```
