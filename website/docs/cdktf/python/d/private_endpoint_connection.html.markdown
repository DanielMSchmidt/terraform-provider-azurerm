...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_private_endpoint_connection_example =
        azurerm.data_azurerm_private_endpoint_connection.DataAzurermPrivateEndpointConnection(self, "example",
            name="example-private-endpoint",
            resource_group_name="example-rg"
        )
        cdktf.TerraformOutput(self, "private_endpoint_status",
            value=cdktf.property_access(data_azurerm_private_endpoint_connection_example.private_service_connection, ["0", "status"])
        )
```
