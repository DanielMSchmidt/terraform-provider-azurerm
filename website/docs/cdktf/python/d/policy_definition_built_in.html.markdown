...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_policy_definition_built_in_example =
        azurerm.data_azurerm_policy_definition_built_in.DataAzurermPolicyDefinitionBuiltIn(self, "example",
            display_name="Allowed resource types"
        )
        cdktf.TerraformOutput(self, "id",
            value=data_azurerm_policy_definition_built_in_example.id
        )
```
