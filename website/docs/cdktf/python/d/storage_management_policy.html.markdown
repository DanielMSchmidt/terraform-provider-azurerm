...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_storage_management_policy_example =
        azurerm.data_azurerm_storage_management_policy.DataAzurermStorageManagementPolicy(self, "example_1",
            storage_account_id=cdktf.Token.as_string(data_azurerm_storage_account_example.id)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        data_azurerm_storage_management_policy_example.override_logical_id("example")
```
