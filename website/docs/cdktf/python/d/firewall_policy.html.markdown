...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_azurerm_firewall_policy_example =
        azurerm.data_azurerm_firewall_policy.DataAzurermFirewallPolicy(self, "example",
            name="existing",
            resource_group_name="existing"
        )
        cdktf.TerraformOutput(self, "id",
            value=data_azurerm_firewall_policy_example.id
        )
```
