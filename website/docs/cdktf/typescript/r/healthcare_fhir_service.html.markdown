...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const azurermHealthcareFhirServiceExample =
      new azurerm.healthcareFhirService.HealthcareFhirService(
        this,
        "example_3",
        {
          ...
          cors: {
            ...
            allowedOrigins: ["https://tfex.com:123", "https://tfex1.com:3389"],
          ...
          workspaceId: cdktf.Token.asString(
            azurermHealthcareWorkspaceExample.id
          ),
        }
      );
    /*This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.*/
...
* `allowedMethods` - (Required) The methods to be allowed via CORS. Possible values are `delete`, `get`, `head`, `merge`, `post`, `options`, `patch` and `put`.
