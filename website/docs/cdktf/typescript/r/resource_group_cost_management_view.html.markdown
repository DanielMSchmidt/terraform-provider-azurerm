...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const azurermResourceGroupCostManagementViewExample =
      new azurerm.resourceGroupCostManagementView.ResourceGroupCostManagementView(
        this,
        "example_1",
        {
          accumulated: false,
    ...
    azurermResourceGroupCostManagementViewExample.overrideLogicalId("example");
  }
}
...
* `chartType` - (Required) Chart type of the main view in Cost Analysis. Possible values are `area`, `groupedColumn`, `line`, `stackedColumn` and `table`.

* `dataset` - (Required) A `dataset` block as defined below.

* `displayName` - (Required) User visible input name of the Cost Management View.
...
* `reportType` - (Required) The type of the report. The only possible value is `usage`.

* `resourceGroupId` - (Required) The ID of the Resource Group this View is scoped to. Changing this forces a new Cost Management View for a Resource Group to be created.
...
* `columnName` - (Required) The name of the column to aggregate. Changing this forces a new Cost Management View for a Resource Group to be created.
