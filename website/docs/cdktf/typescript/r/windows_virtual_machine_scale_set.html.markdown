...
* `galleryApplication` - (Optional) One or more `galleryApplication` blocks as defined below.
...
* `ultraSsdDiskIopsReadWrite` - (Optional) Specifies the Read-Write IOPS for this Data Disk. Only settable when `storageAccountType` is `premiumV2Lrs` or `ultraSsdLrs`.

* `ultraSsdDiskMbpsReadWrite` - (Optional) Specifies the bandwidth in MB per second for this Data Disk. Only settable when `storageAccountType` is `premiumV2Lrs` or `ultraSsdLrs`.
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-d8920c797417c4ac0f3e1575859d63ae06bf4764876c90029292e64fb5120103 -->