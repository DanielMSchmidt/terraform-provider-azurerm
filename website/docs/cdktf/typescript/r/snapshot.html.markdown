...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    const azurermManagedDiskExample = new azurerm.managedDisk.ManagedDisk(
      ...
      {
        createOption: "Empty",
        diskSizeGb: cdktf.Token.asNumber("10"),
    ...
    const azurermSnapshotExample = new azurerm.snapshot.Snapshot(
      ...
      "example_2",
      {
        ...
        sourceUri: cdktf.Token.asString(azurermManagedDiskExample.id),
      }
    );
    /*This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.*/
...
* `incrementalEnabled` - (Optional) Specifies if the Snapshot is incremental.
