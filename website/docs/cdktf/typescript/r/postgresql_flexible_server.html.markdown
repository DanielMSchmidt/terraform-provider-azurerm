    ...
    const azurermPostgresqlFlexibleServerExample =
      new azurerm.postgresqlFlexibleServer.PostgresqlFlexibleServer(
        ...
        "example_6",
        {
          ...
          privateDnsZoneId: cdktf.Token.asString(
            azurermPrivateDnsZoneExample.id
          ),
          resourceGroupName: cdktf.Token.asString(
            azurermResourceGroupExample.name
          ),
...
-> **Note:** To create with `administratorLogin` specified or update with it first specified , `authenticationPasswordAuthEnabled` must be set to `true`.
...
* `version` - (Optional) The version of PostgreSQL Flexible Server to use. Possible values are `11`,`12`, `13`, `14` and `15`. Required when `createMode` is `default`. Changing this forces a new PostgreSQL Flexible Server to be created.
