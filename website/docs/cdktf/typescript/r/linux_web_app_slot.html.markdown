...
* `appServiceId` - (Required) The ID of the Linux Web App this Deployment Slot will be part of.
...
* `servicePlanId` - (Optional) The ID of the Service Plan in which to run this slot. If not specified the same Service Plan as the Linux Web App will be used.
...
* `phpVersion` - (Optional) The version of PHP to run. Possible values are `80`, `81` and `82`.

~> **NOTE:** version `74` is deprecated and will be removed from the provider in a future version.
...
* `identityIds` - (Optional) A list of User Assigned Managed Identity IDs to be assigned to this Linux Web App Slot.
...
* `apiManagementApiId` - (Optional) The API Management API ID this Linux Web App Slot is associated with.

* `apiDefinitionUrl` - (Optional) The URL to the API Definition for this Linux Web App Slot.
...
* `autoSwapSlotName` - (Optional) The Linux Web App Slot Name to automatically swap to when deployment to that slot is successfully completed.
...
* `workerCount` - (Optional) The number of Workers for this Linux App Service Slot.
...
* `appMetadata` - A `appMetadata` block as defined below.
...
* `outboundIpAddressList` - A list of outbound IP addresses - such as `["52.23.25.3", "52.143.43.12"]`
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-1497d0227c5726d6831fa3c979b3a3e42fc9544c1b29f8c9eb4c8479255ca632 -->