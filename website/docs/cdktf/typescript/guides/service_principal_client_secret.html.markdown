...
A Service Principal is an application within Azure Active Directory whose authentication tokens can be used as the `clientId`, `clientSecret`, and `tenantId` fields needed by Terraform (`subscriptionId` can be independently recovered from your Azure account details).
...
* `appId` is the `clientId` defined above.
* `password` is the `clientSecret` defined above.
* `tenant` is the `tenantId` defined above.
...
It's also possible to configure these variables either in-line or from using variables in Terraform (as the `clientSecret` is in this example), like so:
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-2b87e96d966eb04038a9e1e0e57e93d88c6a770d342b7712565bbe2bade190a1 -->